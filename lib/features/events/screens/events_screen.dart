...
Color _severityCardColor(String? sev) {
  ...
  if (s == 'driverstar' ||
      s == 'driver-star' ||
      s == 'driver_star') return Colors.green.shade50;
...
Color _severityAccent(String? sev) {
  ...
  if (s == 'driverstar' ||
      s == 'driver-star' ||
      s == 'driver_star') return Colors.green.shade700;
  if (s == 'neutral') return Colors.blue;
  ...
  Future<void> _fetchInitial() async {
    try {
      ...
      _allSeverities = _uniqueStrings(
        _all.map(
              (e) => _safeStr((e['details'] ?? {})['severityDescription']),
        ),
      );

      _allTypes = _uniqueStrings(
        _all.map(
              (e) => _safeStr((e['details'] ?? {})['typeDescription']),
        ),
      );
  ...
  void _applyAllFilters({bool resetPage = false}) {
    ...
    if (_searchQuery.trim().isNotEmpty) {
      ...
      items = items.where((e) {
        ...
        final severityDesc =
        _safeStr(details['severityDescription']).toLowerCase();

        final city =
        _safeStr((details['location'] ?? {})['city']).toLowerCase();

        final vehicleNum =
        _safeStr((e['vehicle'] ?? {})['vehicleNumber']).toLowerCase();
    ...
    if (_startDateTime != null || _endDateTime != null) {
      items = items.where((e) {
        ...
        if (_startDateTime != null && dt.isBefore(_startDateTime!)) return false;
                        ...
                        Expanded(
                          child: OutlinedButton(
                            ...
                            child: Text(
                              tempStart == null
                                  ? "From Date"
                                  : "From: ${tempStart!.toLocal()}".substring(0, 16),
                            ),
                          ),
                        ),
                      ],
                        ...
                        Expanded(
                          child: OutlinedButton(
                            ...
                            child: Text(
                              tempEnd == null
                                  ? "To Date"
                                  : "To: ${tempEnd!.toLocal()}".substring(0, 16),
                            ),
                          ),
                        ),
                      ],
          ...
          Expanded(
            child: RefreshIndicator(
              ...
              child: _loading
                ...
                itemCount: _visible.length + (_visible.length < _filtered.length ? 1 : 0),
                ...
                padding: const EdgeInsets.fromLTRB(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final severityDesc = _safeStr(details['severityDescription']);
                    final typeDesc = _safeStr(details['typeDescription']);

                    final driverName =
                    "${_safeStr(driver['firstName'])} ${_safeStr(driver['lastName'])}".trim();
                            ...
                            Row(
                              children: [
                                ...
                                Container(
                                  ...
                                  child: Text(
                                    severityDesc.isEmpty ? "-" : severityDesc,
                                    style: TextStyle(
                                      color: accent,
